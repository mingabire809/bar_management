...
$columns = ['nom', 'prenom', 'poste', 'salaire', 'date_embauche'];
$orderColumnName = $columns[$orderColumn];

// Construct the SQL query with search and order
$sql = "SELECT * FROM gestion_employes 
        WHERE nom LIKE :searchValue 
        OR prenom LIKE :searchValue 
        OR poste LIKE :searchValue 
        OR salaire LIKE :searchValue 
        OR date_embauche LIKE :searchValue 
...
$totalRecordsStmt = $pdo->query("SELECT COUNT(*) FROM gestion_employes");
