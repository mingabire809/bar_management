...
$columns = ['nom_boisson', 'quantite_stock', 'prix_achat'];
$orderColumnName = $columns[$orderColumn];

// Construct the SQL query with search and order
$sql = "SELECT * FROM gestion_stock_boisson 
        WHERE nom_boisson LIKE :searchValue 
        OR quantite_stock LIKE :searchValue 
        OR prix_achat LIKE :searchValue 
...
$totalRecordsStmt = $pdo->query("SELECT COUNT(*) FROM gestion_stock_boisson");
